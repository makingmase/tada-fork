...
public class NewSolutionCommand : Command
{
    ...
    public void Execute(string solutionName, DbTypes dbType)
    {
        ...
        // Phase 1: Create all projects (dotnet new commands)
        shell.Execute(Assembly.GetExecutingAssembly().GetResourceText("Scripts.NewSolution.ps1")
            .Replace("$SOLUTION_NAME", solutionName));

        // Strip properties inherited from Directory.Build.props and
        // remove inline Version attributes from PackageReference items added by dotnet new templates.
        // This must happen before dotnet add package commands, which fail if inline versions exist under CPM.
        ...
        foreach (var csprojFile in System.IO.Directory.GetFiles(srcDir, "*.csproj", SearchOption.AllDirectories))
        {
            ...
            FileUpdater.StripPackageReferenceVersions(csprojFile);
        }

        // Phase 2: Add package references, project references, and solution entries
        shell.Execute(Assembly.GetExecutingAssembly().GetResourceText("Scripts.NewSolutionPackages.ps1")
            .Replace("$SOLUTION_NAME", solutionName));
